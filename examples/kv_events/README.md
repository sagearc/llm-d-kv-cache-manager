...
The `online_uds` example is the same as `online` but uses the UDS tokenizer sidecar for tokenization instead of the embedded tokenizer. It scores completions and chat completions against a running KV-cache indexer.
...
go run examples/kv_events/online_uds/main.go
```
...
```bash
# Score a completion request
curl -X POST http://localhost:8080/score_completions \
  -H "Content-Type: application/json" \
  -d '{"prompt": "Hello world", "model": "'"${MODEL_NAME}"'"}' | jq

# Score a chat completion request
curl -X POST http://localhost:8080/score_chat_completions \
  -H "Content-Type: application/json" \
  -d '{"model":"'"${MODEL_NAME}"'", "messages":[{"role":"user","content":"Hello world"}]}' | jq
```
